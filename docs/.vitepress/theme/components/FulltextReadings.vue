...
const locationUrl = (row: Reading, locator: string) => row.source_url + '#' + encodeURIComponent(locator)
  ...
  <section class="fulltext-readings" aria-label="原文阅读发现与限制">
    ...
    <template v-else>
      ...
      <article v-for="row in visible" :key="row.reading_id" class="reading-card">
        ...
        <a class="reading-original" :href="row.source_url" target="_blank" rel="noopener noreferrer">打开所读原文 {{ row.version }} ↗</a>
      </article>
